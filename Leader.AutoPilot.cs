...
public partial class Leader
{
    ...
    public Tile GetTileToPlay(string cardName, bool signature = false)
    {
        ...
        if (card.cardType == "Structure")
        {
            ...
            if (good)
                ...
                column = GM.I.gridWidth - numColumnsDeployable;

            // If the tile has a structure, look one column in.
            while (GM.I.grid[column, row].structure != null)
            {
                if (good)
                    column--;
                else
                ...
                if (column < 0 || column >= GM.I.gridWidth)
                {
    ...
    public Unit FindClosestVisibleEnemy()
    {
        ...
        if (good)
        {
            ...
            foreach (Unit unit in GM.I.GetAllEvilUnits())
            {
        ...
        // Evil.
            ...
            foreach (Unit unit in GM.I.GetAllGoodUnits())
            {
                // Ignore units we can't see.
                if (!unit.IsVisible())
                    continue;
